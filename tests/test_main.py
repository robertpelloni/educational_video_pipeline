...
def test_main_pipeline_execution_with_topic(mock_exists):
    ...
    """
    test_args = ["main.py", "--topic", "Heart", "--output", "dummy_out.mp4", "--skip-upload"]
    ...
    with patch.object(sys, 'argv', test_args), \
         patch("main.fetch_wikipedia_summary", return_value="The heart is an organ.") as mock_fetch, \
         patch("main.generate_script_from_text", return_value=mock_llm_config) as mock_generate_script, \
         patch("main.generate_image_from_prompt") as mock_generate_image, \
         patch("main.generate_all_voiceovers") as mock_generate_audio, \
         patch("main.compile_video") as mock_compile_video, \
         patch("main.upload_video") as mock_upload_video:
        ...
        mock_generate_script.assert_called_once_with("The heart is an organ.", project_id="heart")
        mock_generate_image.assert_called_once_with("Educational illustration regarding: The heart is an organ.", "assets/images/heart_scene_1.png")

        mock_generate_audio.assert_called_once_with(mock_llm_config)
        mock_compile_video.assert_called_once_with(mock_llm_config, output_path="dummy_out.mp4")
