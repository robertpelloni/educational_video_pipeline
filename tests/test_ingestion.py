...
def test_fetch_wikipedia_summary_success(mock_urlopen):
    ...
    mock_data = json.dumps({"extract": "The heart is a muscular organ."}).encode('utf-8')
...
def test_fetch_wikipedia_summary_not_found(mock_urlopen):
    ...
    error_mock = urllib.error.HTTPError(url="", hdrs=None, fp=None, code=404, msg="Not Found")
    mock_urlopen.side_effect = error_mock

    with pytest.raises(ValueError, match="Wikipedia article not found for query: 'NonExistentTopic123'"):
