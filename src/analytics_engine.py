...
def fetch_platform_analytics(project_id: str):
    ...
    if metrics["youtube"]["avg_watch_time_pct"] < 0.4:
        feedback_notes.append("YouTube viewers are dropping off early. Consider a stronger hook.")
    if metrics["tiktok"]["completion_rate"] > 0.4:
        feedback_notes.append("TikTok audience is highly engaged; maintain current pacing.")

    if any(m["audience_sentiment"] == "negative" for m in metrics.values()):
        feedback_notes.append("Sentiment is trending negative; consider adjusting tone or checking factual accuracy.")

    if not feedback_notes:
        feedback_notes.append("Overall performance is stable. Continue current content strategy.")

    logger.info(f"Analytics compiled for '{project_id}': {feedback_notes}")

    return {
        "metrics": metrics,
        "feedback_summary": " ".join(feedback_notes)
    }
